...
const contentAt = (path: string, label: string): Buffer => {
...
const validateLane = (input: {
  ...
  for (const dependencyId of snapshot.codeStartAfter) {
    ...
    const dependency = json(
      contentAt(path, `${taskId}: dependency ${dependencyId}`).toString("utf8"),
      path,
    );
  ...
  const laneBytes = contentAt(lanePath, `${taskId}: lane result`);
  const proofBytes = contentAt(proofPath, `${taskId}: proof`);
  const gateBytes = contentAt(gatePath, `${taskId}: final gate`);
  if (sha256(laneBytes) !== snapshot.laneResultSha256) {
    throw new Error(`${taskId}: lane result digest drift`);
  }
  if (sha256(proofBytes) !== snapshot.proofSha256) {
    throw new Error(`${taskId}: proof digest drift`);
  }
  if (sha256(gateBytes) !== snapshot.gateSha256) {
    throw new Error(`${taskId}: final gate digest drift`);
  }
  const laneContent = laneBytes.toString("utf8");
  const proofContent = proofBytes.toString("utf8");
  const gateContent = gateBytes.toString("utf8");
  ...
  if (snapshot.reproofRequestSha256 !== undefined) {
    ...
    if (
      sha256(readFileSync(priorEvidencePath)) !== request.priorArchiveSha256
    ) {
