...
export const updateProofFromReviewLenses = (
  ...
  atomicWrite(proofPath, jsonContent(withAggregatedReview(proof, aggregate)));
  return aggregate;
};
...
const runCli = (): void => {
  ...
  if (!taskId || !workdir || !evidence) {
    console.error(
      "usage: review-aggregate --task <id> --workdir <absolute-dir> --evidence <absolute-dir>",
    );
    process.exitCode = 2;
    return;
  }
  const aggregate = updateProofFromReviewLenses({ taskId, workdir, evidence });
