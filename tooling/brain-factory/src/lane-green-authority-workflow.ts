...
export const materializeLaneGreenAuthorityWorkflow = (input: {
  readonly path: string;
  readonly sourcePath: string;
  readonly workflowName: string;
}): string => {
  if (!/^BrainBuildTaskS\d{2}T\d{2}Green[0-9a-f]{12}$/.test(input.workflowName))
    throw new Error("lane-green authority workflow name is invalid");
