...
const ownershipRehomeAdmission = (
  ...
  options: {
    ...
    readonly removeEvidence?: "gate" | "lane" | "contract-lens";
  ...
  if (options.removeEvidence === "gate") rmSync(gatePath);
  if (options.removeEvidence === "lane") rmSync(lanePath);
  if (options.removeEvidence === "contract-lens")
    rmSync(join(lenses, "contract.json"));
...
describe("authority refresh admission", () => {
  it("admits only exact succeeded green ownership-rehome evidence", () => {
    ...
    expect(() =>
      assertOwnershipRehomeSourceStatus("succeeded", "S03-T03"),
    ).not.toThrow();
    expect(() =>
      assertOwnershipRehomeSourceStatus("failed", "S03-T03"),
    ).toThrow("ownership-rehome source run must have succeeded");
  }, 15_000);
  ...
  it("rejects missing lenses, wrong pins, refs, and prerequisites", () => {
    for (const removeEvidence of ["gate", "lane", "contract-lens"] as const) {
      expect(() =>
        ownershipRehomeAdmission(fixture(), { removeEvidence }),
      ).toThrow(
        /evidence is missing|ownership-rehome contract review is missing/,
      );
    }
    expect(() =>
      ownershipRehomeAdmission(fixture(), {
      ...
      /ownership-rehome contract review|review lenses|verdict does not match findings/,
    );
    expect(() =>
      ownershipRehomeAdmission(fixture(), {
  ...
  it("wires the explicit CLI mode to the normal conflict-aware workflow", () => {
    ...
    expect(launchSource).toContain('"ownership-rehome"');
