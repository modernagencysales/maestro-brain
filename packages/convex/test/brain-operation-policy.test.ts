...
describe("Brain operation policy", () => {
  ...
  it("preserves independent controls between exact capture and semantic systems", () => {
    ...
    expect(
      ...
      error: { subsystem: "classification" },
    });
  });
  ...
  it("builds durable redacted policy records and audit receipts", () => {
    ...
    expect(operationPolicyFromRecord(row)).toMatchObject({
      state: "paused",
      generation: 2,
      reason: "operator reason redacted",
    });
    expect(row.dataJson).not.toContain("customer prompt canary");
  ...
  it("persists Brain operation policies through registered Confect functions", async () => {
    const program = Effect.gen(function* () {
      ...
      const replayedPause = yield* member.run(
        handlers.setPolicyInternal({
          workspaceId: seeded.workspaceId,
          subsystem: "classification",
          state: "paused",
          ownerUserId: seeded.memberUserId,
          reason: "late duplicate with customer prompt canary",
          idempotencyKey: "classification-pause-1",
          expectedGeneration: 1,
        }),
        BrainOperationPolicyReturn,
      ...
      return { listed, paused, replayedPause, resumed, rows, viewerDenied };
    });
    ...
    expect(JSON.stringify(result.paused)).not.toContain(
      "customer prompt canary",
    );
    expect(result.resumed).toMatchObject({
      state: "enabled",
      generation: 3,
    });
    expect(result.replayedPause).toMatchObject({
      state: "paused",
      generation: 2,
      reason: "operator reason redacted",
    });
