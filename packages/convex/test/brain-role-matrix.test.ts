...
type ProductMembershipSnapshotRowValue = {
  id: string;
  role: string;
  status: string;
...
type ProductInvitationSnapshotRowValue = {
  id: string;
  status: string;
  revokedAt: number | null;
};

type ProductStateSnapshotValue = {
  memberships: ProductMembershipSnapshotRowValue[];
  invitations: ProductInvitationSnapshotRowValue[];
};

type UserMembershipGeneration = {
  id: string;
  role: string;
  status: string;
...
type LifecycleInvitationCaseValue = {
  subject: string;
  workspaceId: string;
  invitationId: string;
};
...
const ProductInvitationSnapshotRow = Schema.Struct({
  id: Schema.String,
  status: Schema.String,
  revokedAt: Schema.NullOr(Schema.Number),
});

const ProductStateSnapshot = Schema.Struct({
  memberships: Schema.Array(ProductMembershipSnapshotRow),
...
const UserMembershipGenerations = Schema.Array(
  UserMembershipGenerationRow,
) as unknown as Schema.Schema<UserMembershipGeneration[], Value>;

const LifecycleInvitationCase = Schema.Struct({
  subject: Schema.String,
  workspaceId: Schema.String,
  invitationId: Schema.String,
});

const LifecycleInvitationCases = Schema.Array(
  LifecycleInvitationCase,
) as unknown as Schema.Schema<LifecycleInvitationCaseValue[], Value>;
  ...
  it("denies accept and cancel for archived Brain and non-active organizations without product writes", async () => {
    const program = Effect.gen(function* () {
      ...
      const cases = yield* confect.run(
        ...
        LifecycleInvitationCases,
      );

      const results: Array<{
        readonly accept: Either.Either<unknown, unknown>;
        readonly cancel: Either.Either<unknown, unknown>;
        readonly before: ProductStateSnapshotValue;
        readonly after: ProductStateSnapshotValue;
      }> = [];
      for (const scenario of cases satisfies ReadonlyArray<LifecycleInvitationCaseValue>) {
        const before = yield* confect.run(
