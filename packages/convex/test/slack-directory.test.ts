...
describe("Slack channel directory contract", () => {
                ...
                .insert({
                  ...
                  status:
                    channel.id === "C_random"
                      ? ("running" as const)
                      : ("idle" as const),
                  cursor: channel.id === "C_random" ? `${lane}-cursor` : null,
                  leaseId: channel.id === "C_random" ? `${lane}-lease` : null,
                  leaseExpiresAt: channel.id === "C_random" ? 1_800 : null,
                  lastProgressAt: channel.id === "C_random" ? 1_500 : null,
                  createdAt: 1_000,
                  updatedAt: 1_000,
                })
                .pipe(Effect.orDie);
      ...
      const rows = yield* confect.run(
        Effect.gen(function* () {
          ...
          const syncs = yield* raw
            .table("channelSyncStates")
            .index("by_channel", (q) =>
              q.eq("channelKey", "slack_agency_acme:C_random"),
            )
            .take(10)
            .pipe(Effect.orDie);
  ...
  it("commits duplicate race audit with one controlled transaction time", async () => {
    ...
    await Effect.runPromise(
      program.pipe(
        Effect.provide(slackDirectoryTestLayer()),
