...
describe("source ledger schema", () => {
  ...
  it("rejects tenant, duplicate receipt, conflicting observation, key, size, and canonicalization violations", () => {
    ...
    expect(() =>
      assertValidSourceLedgerCapture(
        {
          ...capture,
          envelope: { ...capture.envelope, receivedAt: 2_001 },
        },
        { seenTransportDeliveries: seen, verifiedBinding },
      ),
    ).toThrow("DuplicateKeyConflict");
      ...
      assertValidSourceLedgerCapture(
        {
          ...
          observation: {
            ...capture.observation,
            author: { providerUserId: "U_acme", displayName: "Changed" },
          },
        },
        { seenTransportDeliveries: seen, verifiedBinding },
      ),
      ...
      assertValidSourceLedgerCapture(
        {
          ...
          observation: {
            ...capture.observation,
            permalink:
              "https://example.slack.com/archives/C_general/p1680000000000200",
          },
        },
        { seenTransportDeliveries: seen, verifiedBinding },
      ),
    ...
    expect(() =>
      assertValidSourceLedgerCapture(
        {
          ...capture,
          routing: { ...capture.routing, policyEpoch: 8 },
        },
        { seenTransportDeliveries: seen, verifiedBinding },
      ),
    ).toThrow("DuplicateKeyConflict");
  ...
  it("captures tombstones as unreadable lifecycle rows and receipt outcome", () => {
    const rows = buildSourceLedgerRows(
      {
        ...capture,
        envelope: {
          ...capture.envelope,
          transportDeliveryId: "evt_delete",
          receivedAt: 2_000,
        },
        observation: {
          ...capture.observation,
      ...
      {
        existingArtifact: {
          sourceKey: sourceLedgerKeysFor(capture).sourceKey,
          latestProviderOrder: "00000000000000000001",
    ...
    expect(rows.receipt.outcome).toBe("tombstone");
    expect(rows.artifact.lifecycle).toMatchObject({
      state: "deleted_tombstone",
      generation: 2,
      updatedAt: 2_000,
    });
    expect(rows.revision.lifecycle).toEqual(rows.artifact.lifecycle);
    expect(rows.revision.tombstone).toBe(true);
    expect(
      getReadableCurrentSourceRevision(rows.artifact, [rows.revision]),
    ).toBeNull();
  });

  it("keeps active current revisions readable only while the artifact lifecycle is active", () => {
    ...
    expect(
      getReadableCurrentSourceRevision(
        {
          ...rows.artifact,
          lifecycle: { ...rows.artifact.lifecycle, state: "deleted_tombstone" },
        },
        [rows.revision],
      ),
    ).toBeNull();
  ...
  it("rejects stale captures behind the existing source artifact order fence", () => {
    expect(() =>
      buildSourceLedgerRows(
        {
          ...capture,
          envelope: { ...capture.envelope, transportDeliveryId: "evt_stale" },
          observation: {
            ...capture.observation,
            revisionNonce: "delayed-edit",
            providerOrder: "00000000000000000001",
          },
        },
        {
          existingArtifact: {
            sourceKey: sourceLedgerKeysFor(capture).sourceKey,
            latestProviderOrder: "00000000000000000002",
  ...
  it("declares every downstream provider receipt disposition", () => {
    const keys = sourceLedgerKeysFor(capture);
    for (const outcome of [
      ...
      expect(
        Schema.decodeUnknownSync(ProviderEventReceiptRow)({
          ...
          channelKey: "chn_general",
          externalChannelId: "C_general",
          transportDeliveryId: `evt_${outcome}`,
          observationKey: outcome === "rejected" ? null : keys.observationKey,
          sourceKey: outcome === "rejected" ? null : keys.sourceKey,
          sourceRevisionKey:
            outcome === "rejected" ? null : keys.sourceRevisionKey,
          outcome,
          receivedAt: 1_000,
          createdAt: 1_000,
        }).outcome,
