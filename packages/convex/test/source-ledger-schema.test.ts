...
const replayAdmission = (providerEventId: string) => ({
  providerEventId,
  status: "accepted" as const,
  receiptHash: `sha256:${"b".repeat(64)}` as `sha256:${string}`,
});

const makeVerifiedBinding = (
  overrides: Partial<
    Parameters<typeof verifyNativeSlackEnvelope>[0]["binding"]
  > = {},
) => {
  const rawBody = JSON.stringify({
    event_id: overrides.providerEventId ?? "Ev_acme_1",
  });
  const signingSecret = "test-secret";
  const timestampSeconds = 1_700_000_000;
  return verifyNativeSlackEnvelope({
    rawBody,
    signingSecret,
    signature: signSlackRequest(signingSecret, timestampSeconds, rawBody),
    timestampSeconds,
    nowSeconds: timestampSeconds,
    replayAdmission: replayAdmission(overrides.providerEventId ?? "Ev_acme_1"),
    binding: {
...
describe("source ledger schema", () => {
  ...
  it("verifies native Slack signatures, freshness, and durable receipt replay before branding", async () => {
    const t = makeTest();
    const rawBody = JSON.stringify({ event_id: "Ev_native" });
    const signingSecret = "native-secret";
    const timestampSeconds = 2_000;
    const binding = {
      providerEventId: "Ev_native",
      organizationKey: "agency_acme",
      connectionKey: "slack_agency_acme",
      connectionGeneration: 2,
      teamId: "T_acme",
    ...
    const admitReplay = async (providerEventId: string) =>
      await t.run(async (ctx) => {
        const receipts = await testDb(ctx)
        ...
        if (
          ...
          throw new Error("DuplicateKeyConflict");
        return replayAdmission(providerEventId);
      });

    const envelope = verifyNativeSlackEnvelope({
      rawBody,
      signingSecret,
      signature: signSlackRequest(signingSecret, timestampSeconds, rawBody),
      timestampSeconds,
      nowSeconds: timestampSeconds,
      replayAdmission: await admitReplay(binding.providerEventId),
      binding,
    });
    const rows = buildSourceLedgerRows(capture, { verifiedBinding: envelope });
    await t.run(async (ctx) => {
      await testDb(ctx).insert("providerEventReceipts", rows.receipt);
    });
    await expect(admitReplay(binding.providerEventId)).rejects.toThrow(
    ...
    expect(() =>
      verifyNativeSlackEnvelope({
        ...
        timestampSeconds,
        nowSeconds: timestampSeconds,
        replayAdmission: replayAdmission(binding.providerEventId),
        binding,
      }),
    ).toThrow("ChannelAccessLost");
    expect(() =>
