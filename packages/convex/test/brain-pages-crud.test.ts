...
const PageMutationSnapshotSchema = Schema.Struct({
  revisions: Schema.mutable(Schema.Array(Schema.String)),
  audits: Schema.mutable(Schema.Array(PageAuditEventRow)),
});
...
const pageMutationSnapshot = (workspaceId: GenericId<"workspaces">) =>
  Effect.gen(function* () {
    ...
    return { revisions, audits } satisfies PageMutationSnapshot;
  });
...
const collectPageAuditEvents = (workspaceId: GenericId<"workspaces">) =>
  Effect.gen(function* () {
    ...
    const rows = yield* reader
      ...
      .collect();
    return rows.map((row) => {
      const { _id: id, _creationTime: creationTime, ...event } = row;
      void id;
      void creationTime;
      return event;
    });
...
describe("authorized Brain page CRUD", () => {
  ...
  it("requires the stable-key resolver instead of manual WorkOS brain lookup", async () => {
    const program = Effect.gen(function* () {
      const confect = yield* Effect.serviceOptional(
        TestConfect.TestConfect<typeof databaseSchema>(),
      );
      const seeded = yield* confect.run(
        seedBrain({
          role: "editor",
          subject: "missing-agency-key",
          email: "missing-agency-key@example.com",
      ...
      yield* confect.run(
        Effect.gen(function* () {
          ...
          yield* writer
            .table("organizations")
            .patch(seeded.organizationId, { agencyKey: undefined })
            .pipe(Effect.orDie);
          return null;
        }),
        NullReturn,
      ...
      const editor = actor(
        confect,
        "missing-agency-key",
        "missing-agency-key@example.com",
      );
      return yield* editor
        .mutation(refs.public.brain.pages.create, {
          brainKey: matrixBrainKey,
          parentPageKey: null,
          siblingSlug: "manual-bypass",
          sortKey: "0000000001",
          title: "Manual bypass",
          markdown: "# Manual bypass",
    ...
    if (Either.isLeft(result)) {
      expect(result.left).toBeInstanceOf(BrainNotFound);
    }
  });
