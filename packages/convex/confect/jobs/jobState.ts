...
const terminalStatuses: ReadonlySet<SourceJobExecutionStatus> = new Set([
  "succeeded",
  "dead_letter",
  "superseded",
  "revoked",
...
export const tokenHash = (leaseToken: string): string => {
  let hash = 0x811c9dc5;
  for (const char of leaseToken) {
    hash ^= char.charCodeAt(0);
    hash = Math.imul(hash, 0x01000193);
  }
  return `lease-token:${(hash >>> 0).toString(16).padStart(8, "0")}`;
};
...
export const startSourceJob = (
...
): SourceJobResult<DuplicateEffect | LeaseLost> => {
  ...
  if (
    ...
    (job.executionStatus !== "queued" &&
      !(job.executionStatus === "retry_wait" && job.nextRetryAt <= input.now) &&
      !(job.leaseExpiresAt !== undefined && job.leaseExpiresAt < input.now))
  ) {
    return Either.left(
      new LeaseLost({ reason: "active or terminal lease cannot be claimed" }),
    );
  }
...
export const failSourceJob = (
  ...
  if (terminalStatuses.has(job.executionStatus))
    return Either.left(
      new LeaseLost({ reason: "terminal job cannot transition" }),
    );
  if (
  ...
  if (input.kind === "permanent")
    return Either.right(
      deadLetter(job, "PermanentJobFailure", input.reason, input.now),
    );
  return Either.left(new LeaseLost({ reason: "unsupported terminal control" }));
};
