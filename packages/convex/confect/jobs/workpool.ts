...
export const statusSourceJobHandler = async (
  ctx: { readonly db: SourceJobDb },
  args: SourceJobArgs,
) => toStatusOrNull(await findSourceJob(ctx.db, args));
...
const toStatusOrNull = (row: SourceJobRow | null) =>
  row === null ? null : toStatus(row);
...
export const completeSourceJobHandler = async (
  ...
  if (
    ...
    throw new StaleGeneration({ generation: "completionContext" });
