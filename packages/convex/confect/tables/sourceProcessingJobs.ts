...
export const SourceProcessingJobRow = Schema.Struct({
  ...
  organizationUnitIdempotencyKey: Schema.optional(Schema.String),
...
export default Table.make(() => SourceProcessingJobRow)
  .index("by_org_stage_status_next_retry", [
    "organizationKey",
    "stage",
    "executionStatus",
    "nextRetryAt",
  ])
  .index("by_org_effect_key", ["organizationKey", "effectKey"])
  .index("by_org_unit_idempotency_key", [
    "organizationKey",
    "organizationUnitIdempotencyKey",
  ])
  .index("by_org_unit_stage", ["organizationKey", "unitKey", "stage"])
  .index("by_org_lease_expiry", ["organizationKey", "leaseExpiresAt"])
