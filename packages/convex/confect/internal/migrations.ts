...
export const probeExpand = componentMigrations.define({
  ...
  migrateOne: async (ctx, row) => {
    ...
    await ctx.db.patch(row._id, { schemaAfter: "sha256:after" });
  },
});
...
export const stableJson = (value: JsonValue): string => {
  ...
  return `{${Object.keys(value)
    ...
    .map(
      (key) =>
        `${JSON.stringify(key)}:${stableJson(isJsonObject(value) ? (value[key] ?? null) : null)}`,
    )
    .join(",")}}`;
};
...
export type ComponentBatchResult = {
...
export type MigrationCountSummary = {
  readonly scanned: number;
  readonly changed: number | null;
  readonly skipped: number | null;
  readonly failed: number;
  readonly countProvenance: CountProvenance;
};
...
export const makeSettlementInput = (input: {
  readonly args: ExecuteMigrationArgs; readonly lease: Lease; readonly mode: MigrationMode;
  readonly nextCursor: string | null; readonly complete: boolean; readonly processed: number; readonly failed?: number;
}) => ({
  ...input.args,
  mode: input.mode,
...
export const completeActionResult = (
  ...
  childReceiptHash: lease.childReceiptHash ?? "sha256:missing-child",
