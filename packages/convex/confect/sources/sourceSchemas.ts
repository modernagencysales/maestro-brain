...
const ReceiptOutcome = Schema.Literal(
  "inserted",
  "duplicate",
  "tombstone",
  "ignored_bot_output",
...
export const ProviderEventReceiptRow = Schema.Struct({
  ...
  observationKey: Schema.NullOr(StableKey),
  sourceKey: Schema.NullOr(SourceKey),
  sourceRevisionKey: Schema.NullOr(RevisionKey),
...
export const assertValidSourceLedgerCapture = (
  ...
  if (options.existingArtifact) {
    if (options.existingArtifact.sourceKey !== keys.sourceKey)
      throw new DuplicateKeyConflict("DuplicateKeyConflict");
    if (
      decoded.observation.providerOrder <=
  ...
  const outcome = decoded.observation.tombstone
    ? ("tombstone" as const)
    : ("inserted" as const);
  return { outcome, ...keys };
};
...
export const getReadableCurrentSourceRevision = (
  artifact: SourceArtifactRowValue,
  revisions: ReadonlyArray<SourceRevisionRowValue>,
): SourceRevisionRowValue | null => {
  if (artifact.lifecycle.state !== "active") return null;
  return (
    revisions.find(
      (revision) =>
