...
export const sourceLedgerKeysFor = (input: SourceLedgerCaptureInputValue) => {
  const sourceKey = `src_${input.envelope.connectionKey}_${input.envelope.channelKey}_${safeObjectId(input.observation.providerObjectId)}`;
...
export const assertValidSourceLedgerCapture = (
  ...
  if (
    ...
    throw new DuplicateKeyConflict("DuplicateKeyConflict");
  const deliveryKey = `${decoded.envelope.connectionKey}:${decoded.envelope.transportDeliveryId}`;
