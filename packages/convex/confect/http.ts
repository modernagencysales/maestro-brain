  ...
  ...confectManifest.functions
    .filter(
      (entry) =>
        ...
        !isLegacyCreateMarkdownOperation(entry.operationId),
    )
    .map((entry) => ({
...
const filteredOpenApiDocument = (): ReturnType<typeof buildGeneratedOpenApiDocument> => {
  const document = buildGeneratedOpenApiDocument();
  const { ["/api/brain.pages.createMarkdown"]: _legacy, ...paths } = document.paths;
  void _legacy;
