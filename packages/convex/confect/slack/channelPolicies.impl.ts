...
const loadClientBrains = (
  ...
  reader
    ...
    .index("by_organization_kind", (q) =>
      q.eq("organizationId", organizationKey).eq("kind", "client"),
    )
    .collect()
...
const recordPolicyDenial = <E>(
  ...
  runMutation(recordDenialAudit as unknown as Ref.AnyMutation, {
    organizationKey,
    reason,
  }).pipe(
    Effect.orDie,
...
const getChannelPolicyReadModelImpl = FunctionImpl.make(
  ...
  (input) =>
    Effect.gen(function* () {
      ...
      yield* loadPolicyActor(reader, input.organizationKey);
      const [channels, workspaces, policies] = yield* Effect.all([
        loadChannels(reader, input.organizationKey),
        loadClientBrains(reader, input.organizationKey),
