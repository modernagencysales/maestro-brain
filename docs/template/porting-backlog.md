...
## Maestro Brain pattern gap ledger

This ledger registers Maestro Brain product patterns that are not yet generic
app-factory patterns. Each gap is a template finding, not a waiver. State moves
`registered -> product instance -> proven -> promoted | retained product-specific`;
a product implementation alone does not complete the template gap.

| ID                           | Current absence                                                                                             | Generic template lesson                                                                                         | Product-specific first implementation path                                                                               | Promotion criteria                                                                                                  | Owner        | Focused gates                                                                                                                                        |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TB-DEVEX-CONVEX-01`         | No reusable multi-host Convex Codex plugin readiness receipt.                                               | Host bootstrap prerequisites need redacted, reproducible receipts before source work starts.                    | S00-T01 records three verified host/plugin rows in `docs/superpowers/receipts/maestro-brain/convex-plugin-readiness.md`. | Promote a generic host-readiness checklist and receipt schema when another Convex-backed app uses it unchanged.     | foundation   | `rtk codex plugin list`; `rtk pnpm exec prettier --check docs/superpowers/receipts/maestro-brain/convex-plugin-readiness.md`; `rtk git diff --check` |
| `TB-AUTHKIT-01`              | AuthKit identity is not a complete reusable TanStack Start + Convex pattern.                                | Auth must bind server sessions, Convex claims, and client providers without demo bypass.                        | S01-T01 wires real WorkOS/AuthKit identity through web and Convex config.                                                | Promote once sign-in, callback, session refresh, Convex auth config, and negative auth tests are generator-ready.   | identity     | `rtk pnpm --dir apps/web test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:auth-demo-bypass`                                             |
| `TB-BRAIN-UI-01`             | No reusable Brain workspace page-tree/editor/history surface pattern.                                       | Knowledge UI needs stable page identity, responsive editing, citations, and restore shells separated by layers. | S03 builds the Maestro Brain product shell, page tree, editor pane, evidence drawer, and history/review components.      | Promote after blocks/adapters are generic and route-local business logic is removed.                                | web          | `rtk pnpm --dir apps/web test`; `rtk pnpm check:route-tree`; `rtk pnpm check:frontend-effect-boundary`                                               |
| `TB-NANGO-SLACK-01`          | No generic Nango Slack connection boundary or webhook-reference pattern.                                    | Provider OAuth/proxy ownership and native webhook verification must be isolated from product routing.           | S04 adds Nango Connect, Slack connection persistence, channel directory, manifest pinning, and webhook security.         | Promote once provider interfaces, fake adapters, manifest receipt, and signed-webhook negative matrix are reusable. | slack-source | `rtk pnpm --dir packages/integrations test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:provider-boundary`                               |
| `TB-SOURCE-01`               | Source ledger, normalization, ordering, and immutable source-unit assembly are absent as a generic pattern. | Source ingestion should preserve exact raw observations and deterministic derived units before model calls.     | S05 adds Slack source ledgers, atomic capture, ordering, assembler, and direct/capture-only dispatch.                    | Promote after source-type contracts, ordering tests, and deterministic replay fixtures apply to a second provider.  | slack-source | `rtk pnpm --dir packages/convex test`; `rtk pnpm --dir packages/integrations test`; `rtk pnpm check:logging-boundary`                                |
| `TB-SOURCE-LIFECYCLE-01`     | Lifecycle propagation across raw, derived, indexed, queued, delivered, and exported data is doctrine-only.  | Privacy actions need current-use revocation plus descendant receipts, not isolated delete helpers.              | S07 adds lifecycle envelopes, holds, purge/DSAR propagation, and recovery receipts.                                      | Promote when trigger matrix, descendant registry, and hold-aware redaction are reusable across source types.        | lifecycle    | `rtk pnpm --dir packages/convex test`; `rtk pnpm check:schema-migration-notes`; `rtk pnpm check:logging-boundary`                                    |
| `TB-AUTHORIZED-KNOWLEDGE-01` | Authorized versioned knowledge reads/writes are not a reusable Confect capability pattern.                  | Knowledge mutations must use stable public keys, tenant checks, revisions, citations, and typed failures.       | S02 replaces fixtures with authorized Brain pages, revisions, citations, and editor fences.                              | Promote after generated specs, repos, and tests cover page/revision/citation contracts without product names.       | brain        | `rtk pnpm --dir packages/convex test`; `rtk pnpm check:confect-contracts`; `rtk pnpm check:access-audit-events`                                      |
| `TB-STRUCTURED-LLM-01`       | Provider-neutral structured LLM gateway with receipts, budgets, and typed failures is missing.              | Model calls are semantic adapters with schema-constrained IO, hashes, budgets, and redacted receipts.           | S08-T01 adds the Maestro Brain structured LLM gateway and model-call receipts.                                           | Promote when a second workflow uses the same gateway and budget/egress tests without product assumptions.           | cognition    | `rtk pnpm --dir packages/integrations test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:logging-boundary`                                |
| `TB-INTERNAL-WORKFLOW-01`    | Workflow generator has no internal-only review-first mode.                                                  | Internal workflows need generated gather/request/validate/commit seams without public route exposure.           | S08-T02 extends the workflow playbook/generator; S08-T03/T04 instantiate classification and maintenance.                 | Promote when generator output, docs, and gates are stable for another internal workflow.                            | cognition    | `rtk pnpm --dir tooling/generators test`; `rtk pnpm check:generators`; `rtk pnpm test:stack`                                                         |
| `TB-ASYNC-SEARCH-01`         | Search seam is synchronous/stubbed and lacks authorized projection contracts.                               | Search should be asynchronous Effect semantics with tenant-scoped active projections.                           | S09-T01 adds the async Effect service and fake/live parity; S09-T02 adds authorized workspace projections.               | Promote after package tests prove provider-neutral async semantics and projection isolation.                        | retrieval    | `rtk pnpm --dir packages/search test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:layer-boundaries`                                      |
| `TB-HEADLESS-01`             | API/CLI/MCP one-Brain service-principal pattern is incomplete.                                              | Headless surfaces must dispatch through the same capability registry with one-Brain viewer ceilings.            | S11 adds API keys, bearer principal resolution, reviewed read/Ask registry, and stateless MCP.                           | Promote when registry/schema parity, protocol negatives, and copyable client config are generic.                    | headless     | `rtk pnpm --dir apps/cli test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:headless-surface-contract`                                    |
| `TB-BRAIN-EXPORT-01`         | Deterministic stable-key export codec and lifecycle-fenced export jobs are absent.                          | Exports need byte-identical bundles, temporary artifacts, expiry, purge, and no import side effects.            | S12 adds the export codec, jobs, storage boundary, and settings UI.                                                      | Promote when codec package and job/storage flow are reusable for another knowledge domain.                          | export       | `rtk pnpm --dir packages/template-core test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:logging-boundary`                               |
| `TB-DEPLOY-ISOLATION-01`     | Staging and production isolation is not a reusable release pattern.                                         | Tenant deploys must use distinct backends, credentials, callbacks, and promotion/rollback receipts.             | S00-T03 repairs deployment config/scripts and records `deployment-isolation.md`.                                         | Promote after deploy doctor, staged-SHA promotion, rollback planning, and no-demo-seed checks are generic.          | foundation   | `rtk pnpm --dir tooling/release test`; `rtk pnpm deploy:doctor staging`; `rtk pnpm deploy:doctor production`                                         |
| `TB-AUTHORIZED-TENANCY-01`   | Stable agency/Brain keys and authorized provisioning are product-specific work, not a template pattern.     | Tenant identity needs stable public keys, role-aware provisioning, and no unfiltered workspace queries.         | S01-T02/T03 add WorkOS organization binding, stable keys, and authorized Brain provisioning/listing.                     | Promote after schemas, provisioning capabilities, and cross-tenant denial tests are reusable.                       | identity     | `rtk pnpm --dir packages/convex test`; `rtk pnpm --dir apps/web test`; `rtk pnpm check:access-audit-events`                                          |
| `TB-ACCESS-UI-01`            | Member/settings/admin access UI is not a reusable audited surface pattern.                                  | Privileged UI must reflect server roles and audit both denials and successes.                                   | S01-T04 applies RBAC to members, settings, invites, and privileged actions.                                              | Promote when settings blocks, adapters, and audit capability contracts are product-neutral.                         | identity     | `rtk pnpm --dir apps/web test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:frontend-effect-boundary`                                     |
| `TB-EVALS-01`                | Frozen semantic/security eval suites are not wired as reusable release gates.                               | Model features need versioned fixtures, thresholds, reports, and drift hashes.                                  | S13-T01 builds Maestro Brain eval suites for answers, maintenance, multilingual, and injection cases.                    | Promote when eval runner/report format and fixture hashing serve another product workflow.                          | operations   | `rtk pnpm --dir tooling/evals test`; `rtk pnpm evals`; `rtk pnpm check:logging-boundary`                                                             |
| `TB-OPERATIONS-01`           | Redacted telemetry, budgets, kill switches, capacity, and recovery drills are not a generic ops kit.        | Operations must prove redaction, admission control, spend/rate/storage budgets, fairness, and recovery.         | S13-T02/T03/T04 add capacity harness, metrics, policies, alerts, and dashboard/runbooks.                                 | Promote when observability services, policy gates, and capacity fixtures are reusable without Maestro Brain names.  | operations   | `rtk pnpm --dir tooling/evals test`; `rtk pnpm --dir packages/observability test`; `rtk pnpm check:logging-boundary`                                 |
| `TB-RELEASE-EVIDENCE-01`     | Staging/pilot/launch/rollback evidence packet is not a reusable release template.                           | Launch evidence needs source-of-truth CI, provider smokes, pilot metrics, approvers, and rollback proof.        | S14 records Maestro Brain deployment, incident rollback, limits, privacy/security, checklist, and launch receipt.        | Promote after a second launch can reuse the packet structure and no secret/customer text rules.                     | release      | `rtk pnpm deploy:doctor production`; `rtk proxy .buildkite/scripts/production-promote.sh`; `rtk just verify-full`                                    |

### StackPlan projection contract

A just-in-time StackPlan is authority only after
`rtk pnpm stack:check <absolute-temp-plan.json>` passes against the current
checkout. The temporary JSON stays outside the repo and is deleted after its
SHA-256, depth, source estimates, and fail/pass transcript are preserved in the
stack receipt.

Required slice fields are `id`, `branch`, `intention`, `layers`,
`contractRiskIds`, `workPackages`, `taskRefs`, `rationale`, and `estLines`.
Required stack fields are `feature`, `slices`, and `allTaskRefs`. Every
`workPackages[]` entry declares one `kind` (`fixture-to-real`,
`pattern-instance`, or `template-gap`), a non-empty `target`, non-empty
`followUpGates`, and the kind-specific generator, fixture, or backlog/promotion
path metadata.

Source-line review is split, not waived: each stack reports hand-authored source
lines separately from generated, test, and documentation review totals. The
300-source-line estimate and four-slice depth caps fail closed; a large stack is
split before implementation rather than exempted after the fact.
...
87. **Migrations component pattern** — LOW — partial. S00-T04 adds an
    internal-only Confect harness, reserved migration names, bounded dry-run /
    execute batches, component-cursor resume receipts, and docs in
    `docs/product/maestro-brain-migrations.md`. Remaining template promotion:
    graduate the wrapper after two product migrations use it and central codegen
    owns generated Convex/Confect output.
