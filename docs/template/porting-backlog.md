...
| `TB-ASYNC-SEARCH-01`         | Search seam is synchronous/stubbed and lacks authorized projection contracts.                               | Search should be asynchronous Effect semantics with tenant-scoped active projections.                           | S09-T01/T02 migrate search and add authorized workspace projections.                                                     | Promote after package tests prove provider-neutral async semantics and projection isolation.                        | retrieval    | `rtk pnpm --dir packages/search test`; `rtk pnpm --dir packages/convex test`; `rtk pnpm check:layer-boundaries`                                      |
