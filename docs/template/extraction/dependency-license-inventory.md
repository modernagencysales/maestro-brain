...
| Notion Kit | Blocks and settings primitives | Required review | Any private tarball must be explicitly approved and listed here. |
...
artifact must have an approval note before it enters the repo. The first known
candidate is a Notion Kit i18n tarball if the frontend still requires it.
