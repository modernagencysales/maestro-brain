...
| Convex                    | `CONVEX_DEPLOYMENT`, `VITE_CONVEX_URL`, `CONVEX_SITE_URL`, `MAESTRO_BRAIN_STAGING_CONVEX_DEPLOY_KEY`, `MAESTRO_BRAIN_PRODUCTION_CONVEX_DEPLOY_KEY`, `MAESTRO_BRAIN_STAGING_CALLBACK_ORIGIN`, `MAESTRO_BRAIN_PRODUCTION_CALLBACK_ORIGIN`, `MAESTRO_BRAIN_RELEASE_SIGNER`, `MAESTRO_BRAIN_RELEASE_SIGNING_KEY_ID`, `MAESTRO_BRAIN_RELEASE_SIGNING_SECRET` | Backend owner           | Convex runtime, Confect generated refs, web client, deploy isolation     | Local web mode leaves `VITE_CONVEX_URL` blank; deploy examples use fake local keys only                                                                       | Staging and production must use distinct Convex deployments, deploy keys, URLs, callback origins, and release packets before promotion                                | Rotate deployment admin access on team changes; regenerate URL mapping on deployment changes                      |
