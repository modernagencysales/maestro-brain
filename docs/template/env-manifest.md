...
| Nango Connect             | `NANGO_SECRET_KEY`, `NANGO_CONNECT_INTEGRATION_ID`, `NANGO_CONNECT_PUBLIC_KEY`                                                                                                                                                                                                                                                                                                   | Integrations owner      | Slack and call-transcript authorization, sync-model reads, and provider proxy calls    | Fake values create deterministic local Connect sessions and never require provider credentials                                                                | Required when a live Slack or transcript connector is enabled; secret key stays server-side and every provider access/refresh token remains in Nango                  | Rotate before live enablement, after integration admin changes, and after any suspected callback/session exposure |
