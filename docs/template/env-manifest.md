...
| Convex                    | `CONVEX_DEPLOYMENT`, `VITE_CONVEX_URL`, `CONVEX_SITE_URL`                                                                            | Backend owner           | Convex runtime, Confect generated refs, web client                     | Points at fake/example URLs until a dev deployment is created                             | Required for any backend-backed fork; production must use the production Convex deployment                             | Rotate deployment admin access on team changes; regenerate URL mapping on deployment changes                      |
