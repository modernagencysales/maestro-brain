...
| Convex                    | `CONVEX_DEPLOYMENT`, `VITE_CONVEX_URL`, `CONVEX_SITE_URL`, `MAESTRO_BRAIN_API_KEY`, `MAESTRO_BRAIN_STAGING_CONVEX_DEPLOY_KEY`, `MAESTRO_BRAIN_PRODUCTION_CONVEX_DEPLOY_KEY`, `MAESTRO_BRAIN_STAGING_CALLBACK_ORIGIN`, `MAESTRO_BRAIN_PRODUCTION_CALLBACK_ORIGIN`, `MAESTRO_BRAIN_RELEASE_SIGNER`, `MAESTRO_BRAIN_RELEASE_SIGNING_KEY_ID`, `MAESTRO_BRAIN_RELEASE_SIGNING_SECRET` | Backend owner           | Convex runtime, Confect generated refs, remote Brain CLI, web client, deploy isolation                    | Local web mode leaves `VITE_CONVEX_URL` blank; remote CLI needs a real Brain bearer key                                                                       | Staging and production must use distinct Convex deployments, deploy keys, URLs, callback origins, and release packets before promotion                                                                     | Rotate deployment admin access on team changes; regenerate URL mapping on deployment changes                                 |
...
| Nango Connect             | `NANGO_SECRET_KEY`, `NANGO_WEBHOOK_SIGNING_KEY`, `NANGO_CONNECT_INTEGRATION_ID`, `NANGO_CONNECT_PUBLIC_KEY`                                                                                                                                                                                                                                                                      | Integrations owner      | Slack and call-transcript authorization, sync-model reads, webhook verification, and provider proxy calls | Fake values create deterministic local Connect sessions and webhook verification without provider credentials                                                 | Required when a live Slack or transcript connector is enabled; secrets stay server-side, the webhook signing key comes from Nango Environment Settings, and provider access/refresh tokens remain in Nango | Rotate before live enablement, after integration or webhook admin changes, and after any suspected callback/session exposure |
...
| Cloudflare                | `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_PAGES_PROJECT`, `CLOUDFLARE_PAGES_BRANCH`, `TEMPLATE_HOSTED_URL`, `MAESTRO_BRAIN_STAGING_CLOUDFLARE_API_TOKEN`, `MAESTRO_BRAIN_STAGING_CLOUDFLARE_ACCOUNT_ID`, `MAESTRO_BRAIN_PRODUCTION_CLOUDFLARE_API_TOKEN`, `MAESTRO_BRAIN_PRODUCTION_CLOUDFLARE_ACCOUNT_ID`                                                    | Deploy owner            | hosted smoke, Pages deploy, future Workers decision gate                                                  | Fake values are enough for local docs and generator output; hosted smoke needs a real URL                                                                     | Required for hosted environments; staging and production tokens must be scoped to their own Pages projects                                                                                                 | Rotate token on deploy-admin changes; review Pages/Workers mapping before promotion                                          |
...
The template CI uses isolated staging and production Convex deployments, URLs,
deploy-key names, callback-origin slots, release packet signing keys, and
Cloudflare credential names. Tenant deploy scripts refuse shared backend config,
run `deploy-doctor` for the target environment, never seed `demo/showcase`, and
require the exact staged release packet before production promotion. Forks must
keep provider callback registrations aligned with the corresponding staging or
production origin.
...
Transcript providers do not add provider token env vars to Brain. Configure the
provider credential fields in Nango, keep only `NANGO_SECRET_KEY` in the server
secret store, and follow
