...
deploy-key names, callback-origin slots, and Cloudflare credential names. Tenant
deploy scripts refuse shared backend config, run `deploy-doctor` for the target
environment, never seed `demo/showcase`, and require the exact staged release
packet before production promotion. Forks must keep provider callback
registrations aligned with the corresponding staging or production origin.
