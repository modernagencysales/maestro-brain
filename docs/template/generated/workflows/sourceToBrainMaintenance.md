...
- `packages/convex/convex/workflowRunners/sourceToBrainMaintenance.ts`: plain
  Convex `defineWorkflow` durable replay handler generated by the workflow
  scaffold and centralized Confect/Convex integration, not retained in the lane
  hand-authored Confect freshness delta.
...
2. Run `pnpm --dir packages/convex exec convex codegen` after writing the
   ...
   `workflowContracts.sourceToBrainMaintenance` wrappers; integration owns the
   reproduced Convex runner delta when Confect sync refreshes generated output.
