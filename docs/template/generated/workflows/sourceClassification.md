...
- `packages/convex/confect/workflows/sourceClassification.graph.ts`: durable
  server-gather, classify, admin-review, current-authority, mechanical-commit,
  and receipt graph data.
...
4. The approval event carries an admin/owner review command; confidence never
   advances the graph.
5. The runner starts from a source-unit revision key, gathers the immutable
   request server-side, rechecks current authority after review, maps the
   request to `classifySourceUnit`, and maps the reviewed proposal to the
   mechanical `commitSourceRoute` capability.
