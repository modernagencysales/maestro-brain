...
The current lifecycle planner covers existing workspace-owned resources only:
workspaces, workspace members, Brain pages, workflow runs/stages/events/evidence
snapshots/context manifests, usage events, credit ledger, API keys, and
invitations. Deletion requires the typed confirmation phrase generated by the
planner.
