...
create API, CLI, MCP, OpenAPI, or headless descriptors; callers must be reviewed
internal capabilities, jobs, or workflow runners that already own the tenant and
lifecycle fence. Generated capability nodes are only usable when their registry
entries include a concrete `buildArgs` mapper for the target internal capability
ref.
