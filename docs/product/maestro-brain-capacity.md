...
- every runnable channel advances by exact channel id in each 60-second fairness
  window unless explicitly provider-rate blocked;
- no channel id may miss two consecutive windows without progress;
...
- dropped events, queue overflow, attempt/effect mismatch, cross-Brain leak
  effects, and tenant canary bypasses are zero;
