...
**Audited base:** `f5a77ade`; live activation evidence is recorded in
...
| Terminal-first Codex, Claude Code, and Cowork onboarding                                   | Public CLI `0.1.2`; live linked `apero` key; green `doctor`; key-free runtime descriptors; child-process key injection; `docs/team-onboarding.md`                                                    | **Proven**     | Publish the follow-up CLI, then each real pilot links, launches Codex/Claude with `maestro-brain run`, and records one Ask receipt. Cowork import may remain manual. |
...
| Source changes, removals, and access loss propagate inside a declared freshness window     | Immutable revisions, successful-close removal inference, failed-run preservation, persisted approved scopes, hourly reconciliation, connection fail-closed reads, and health                         | **Partial**    | Measure real provider-to-index lag against Apero's freshness targets and add provider cursors only where hourly reconciliation misses them.                          |
