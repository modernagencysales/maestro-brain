...
| Terminal-first Codex, Claude Code, and Cowork onboarding                                   | Public CLI `0.1.1`; live linked `apero` key; green `doctor`; locally generated key-free Codex, Claude, Cowork, and Ask Apero artifacts; `docs/team-onboarding.md`                                            | **Proven**     | Each real pilot user links, restarts their runtime, and records one Ask receipt. Cowork descriptor import remains manual when discovery fails.                |
...
| Human-maintained company context can enter and change safely                               | Web editor, immutable revisions, optimistic concurrency, backend-unique slugs/import identities, and provenance-fenced recursive Markdown upsert with fail-closed collision/resume behavior                  | **Partial**    | Publish CLI `0.1.2` and record a live repeat-import receipt; local file deletion intentionally does not archive a Brain page.                                 |
