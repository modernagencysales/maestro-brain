...
Emergency `disabled` is for operator-controlled stop conditions. Policy updates
retire the prior active row for the same workspace/subsystem, expired rows
evaluate as the default enabled policy, and each accepted change writes a
redacted access-audit receipt. Re-enabling a stale generation returns
`RecoveryGenerationMismatch`; running a disabled subsystem returns
`SubsystemDisabled`; stale or under-privileged operators return
`OperatorForbidden`.
...
- Kill-switch drills: capture, backfill, classification, maintenance, Ask, Slack
  delivery, MCP, export, and lifecycle are enumerated independently in the
  policy contract and focused policy tests.
- Canary results: observability tests cover prompt, source, token, and header
  redaction canaries recursively.
- Budget enforcement: observability tests cover model token/spend, Slack rate,
  storage, queue-depth, and channel-count caps with typed `BudgetExceeded`
  results.
- Redacted audit samples: focused Convex policy tests persist policy changes
  through generated Confect refs and assert access audit receipts omit operator
  reason text.
