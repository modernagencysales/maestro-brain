...
payload and does not write coordinator rows, target rows, or receipts. Any other
component failure follows the production failure path: failed child and
`failure_checkpoint` receipts are durably appended before `MigrationBatchFailed`
returns.
