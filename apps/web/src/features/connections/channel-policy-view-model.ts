...
export type ChannelPolicyChannel = {
  ...
  readonly routingPolicyEpoch?: number | undefined;
  readonly deliveryGeneration?: number | undefined;
};
...
export const buildChannelPolicyDialogState = (input: {
  ...
  const targetCount = input.targetBrainKeys?.length ?? 0;
  const warnings = [
    ...
    ...(input.selectedChannelCount === 0
      ...
      : []),
    ...(input.routingMode === "direct" && targetCount !== 1
      ? ["Direct routing requires exactly one Client Brain target."]
      : []),
    ...(input.routingMode === "classify" && targetCount === 0
      ? ["Classify routing requires at least one Client Brain target."]
      : []),
    ...(input.routingMode === "capture_only" && targetCount > 0
    ...
    changes: input.selectedChannelKeys.flatMap((channelKey) => {
      ...
      return !channel?.isJoined
        ...
        : [
            {
              channelKey,
              expectedRoutingPolicyEpoch: channel.routingPolicyEpoch,
              expectedDeliveryGeneration: channel.deliveryGeneration,
              routing: {
                mode: input.routingMode,
                targetBrainKeys: [...input.targetBrainKeys],
              },
              delivery: {
                mode: channel.isSlackConnect
                  ? "capture_only"
                  : input.deliveryMode,
              },
            },
