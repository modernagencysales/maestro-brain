...
const actionLabels: Record<MaintenanceReviewStatus, string> = {
  proposed_noop: "Accept no-op",
  awaiting_review: "Review required",
  published: "Published",
  edited_and_published: "Published",
  rejected: "Rejected",
  revoked: "Revoked",
...
export const describeMaintenanceProposal = (
  ...
  actionLabel: actionLabels[proposal.status],
  blockedReason:
...
export const buildMaintenanceReviewModel = ({
  ...
  autopilotControl:
    viewerRole !== "admin" && viewerRole !== "owner"
      ? "hidden"
      : proposals.some((proposal) => proposal.autopilotEligible)
        ? "eligible"
        : "review_first_locked",
