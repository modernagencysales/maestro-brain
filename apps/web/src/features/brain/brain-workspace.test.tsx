...
describe("BrainWorkspace", () => {
  ...
  it("keeps a newer rejected BlockNote save conflict when an older deferred save resolves later", () => {
    let latestAttemptId = 0;
    const olderAttemptId = ++latestAttemptId;
    const newerAttemptId = ++latestAttemptId;
    let conflict: "stale_revision" | undefined;
    let savedRevisionKey: string | null = null;

    if (latestAttemptId === newerAttemptId) {
      conflict = reduceSaveConflict({
        status: "typed_failure",
        error: { _tag: "StaleRevision" },
      });
    }
    if (latestAttemptId === olderAttemptId) {
      savedRevisionKey = reduceWorkspaceEditorRevisionFenceAfterSave(
        {
          status: "ready",
          mode: "edit",
          mutation: "success",
    ...
    expect(savedRevisionKey).toBeNull();
    expect(conflict).toBe("stale_revision");
  });

  it("keeps a newer rejected BlockNote save conflict when an older deferred save resolves first", () => {
    let latestAttemptId = 0;
    const olderAttemptId = ++latestAttemptId;
    const newerAttemptId = ++latestAttemptId;
    let conflict: "stale_revision" | undefined;
