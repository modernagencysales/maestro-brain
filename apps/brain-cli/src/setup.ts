...
const installSkill = (root: string, path: string, source: string): Artifact => {
  const destination = join(root, path);
  const stat = lstatSync(destination, { throwIfNoEntry: false });
  if (stat !== undefined) {
...
export const setupProject = (input: {
  ...
  const artifacts: Artifact[] = [
    ...
    installSkill(root, ".agents/skills/ask-apero", input.assetDirectory),
    installSkill(root, ".claude/skills/ask-apero", input.assetDirectory),
  ];
